...
model using the Thermometer method for image binarization.
...
def binarize_list(image):
    m = np.zeros((28*28*resolucao, ), dtype=np.int8)
    image2 = cv.GaussianBlur(image, (9, 9), 0)
    i = 0
    for ele in image2:
        aux = thermometer.encode(ele)
        for ele2 in aux:
            for ele3 in ele2:
                m[i] = ele3
                i = i + 1
...
def binary_encoder_list(images):
    aux = [binarize_list(image) for image in images]
    return aux
...
resolucao = 16  
thermometer = ThermometerEncoder(minimum=0, maximum=255, resolution=resolucao)
number_testes = 60000
...
# # --------------------------------------- TEST -------------------------------------
# model = wp.Wisard(42)  # represents n-tuple size
...
print("Thermometer method")
with open('results/kmnist-thermometer.csv', 'w+') as file:
...
#     model_clus.train(train_bin_list, train_label)  # .tolist())
